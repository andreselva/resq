...
export class EventsProcessorService {
    ...
    async process(event: EventEntity) {
        ...
        const lonDelta = radiusMeters / (approximateLatitude * Math.cos(event.latitude * Math.PI) / 180);
        ...
        const volunteers = await this.repository.getVolunteersForEvent(event, minLon, maxLon, minLat, maxLat, radiusMeters);
