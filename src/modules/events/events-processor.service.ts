...
export class EventsProcessorService {
    ...
    async process(event: EventEntity) {
        const [ minLat, maxLat, minLon, maxLon, radiusMeters ] = this.processCoordinates(event.impact_radius, event.latitude, event.longitude)
        ...
        people.map(p => {
            if (p.type === UserType.NORMAL) {
                ...
                this.logger.warn(`Pessoa de tipo incorreto para atribuição de grupo de ajuda ou notificação de risco: ${p.type}`);
            }
        })
     ...
     * @returns 
     */
    processCoordinates(impactRadius: number, eventLatitude: number, eventLongitude: number) {
